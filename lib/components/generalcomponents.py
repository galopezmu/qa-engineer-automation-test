...
class GeneralComponents(object):
    ...
    def wait_until_element_is_present(self, web_element, timeout=Constants.MEDIUM_WAIT):
        ...
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.presence_of_element_located(web_element), error_message
            )
        except TimeoutException as e:
            raise e

    def wait_until_element_is_clickable(
        self, web_element, timeout=Constants.MEDIUM_WAIT
    ):
        ...
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.element_to_be_clickable(web_element), error_message
            )
        except TimeoutException as e:
            raise e

    def wait_until_element_is_not_present(
        self, web_element, timeout=Constants.MEDIUM_WAIT
    ):
    ...
    def wait_until_title_is(self, title, timeout=Constants.MEDIUM_WAIT):
        ...
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.title_is(title), error_message
            )
        except TimeoutException as e:
            raise e
    ...
    def wait_until_title_contain(self, title, timeout=Constants.MEDIUM_WAIT):
        error_message = (
            f'The title page not contain the "{title}" title after of {timeout} seconds'
        )
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.title_contains(title), error_message
            )
        except TimeoutException as e:
            raise e

    def wait_until_text_is_present_in_value(
        self, locator, text, timeout=Constants.MEDIUM_WAIT
    ):
        error_message = f"Text in value is not present after of {timeout} seconds"
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.text_to_be_present_in_element_value(locator, text), error_message
            )
        except TimeoutException as e:
            raise e
    ...
    def wait_until_element_is_not_visible(
        ...
        try:
            return WebDriverWait(self.web_driver, timeout, 1).until(
                EC.invisibility_of_element(web_element), error_message
            )
        except TimeoutException as e:
            raise e
    ...
    def wait_until_element_is_visible(self, web_element, timeout=Constants.MEDIUM_WAIT):
        ...
        try:
            return WebDriverWait(self.web_driver, timeout, 1).until(
                EC.visibility_of(web_element), error_message
            )
        except TimeoutException as e:
            raise e

    def wait_until_two_elements_are_present(
        self, web_element_1, web_element_2, timeout=Constants.MEDIUM_WAIT
    ):
        error_message = f"The elements took more than {timeout} seconds longer than the configured time to be present in the DOM."
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.all_of(
                    EC.presence_of_element_located(web_element_1),
    ...
    def wait_until_two_elements_are_clickable(
        self, web_element_1, web_element_2, timeout=Constants.MEDIUM_WAIT
    ):
        ...
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.all_of(
                    EC.element_to_be_clickable(web_element_1),
                    EC.visibility_of_element_located(web_element_2),
                ),
                error_message,
            )
        except TimeoutException as e:
    ...
    def wait_until_url_is(self, url, timeout=Constants.MEDIUM_WAIT):
        ...
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.url_to_be(url), error_message
            )
        except TimeoutException as e:
            raise e
    ...
    def wait_until_url_contains(self, contains, timeout=Constants.MEDIUM_WAIT):
        ...
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.url_contains(contains), error_message
            )
        except TimeoutException as e:
            raise e
    ...
    def is_element_present_in_component(list_element, context, component_name):
        ...
        for element in elements:
            component_elements = context.current_page.get_component_elements_per_name(
                component_name
            )
            web_element = context.browser.find_elements(
                component_elements.__dict__.get(element)
            )
    ...
    def click_component_by_name(context, component_name, name_element):
        ...
        web_element = context.browser.find_element(
            component_elements.__dict__.get(name_element)
        )
    ...
    def is_enabled_in_component(context, component_name, element_name) -> bool:
        ...
        web_element = context.browser.find_element(
            component_elements.__dict__.get(element_name)
        )
    ...
    def wait_until_element_searched_is_present(
        self, web_element, expected_text, timeout=Constants.MEDIUM_WAIT
    ):
        error_message = f"The element took more than {timeout} seconds longer than the configured time to be present in the DOM."
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.text_to_be_present_in_element(web_element, expected_text),
                error_message,
            )
        except TimeoutException as e:
            raise e
    ...
    def get_element_list_from_component(context, component_name, element_name):
        ...
        element_list = context.browser.find_elements(
            component_elements.__dict__.get(element_name)
        )
        for element in element_list:
            if GeneralComponents.is_enabled_in_component(
                context, component_name, element_name
            ):
